...
- 先判断当前项目处于哪一类状态，再决定工作方式，不要把所有任务都当成“新项目启动”：
  ...
  - 如果项目已经做了一部分，默认先识别“现在做到哪一层、主链是否跑通、还缺什么闭环、用户当前理解卡在哪”，不要假设需要推倒重来。
- 对于已经做了一部分的项目，开始工作前优先输出一份“当前阶段地图”，至少说明：
  ...
  - 下一个最值得做的最小闭环是什么
  - 为什么先做它，而不是别的
- 只有在仓库确实缺失架构主线、或用户明确要求重新设计时，才进入完整的“项目设计 -> 分阶段 -> Day1”启动流程。
- 如果需要做项目设计，设计要结合用户方向偏好、当前仓库现状和现有技术栈，先提出一个架构合理、技术栈完整、能真实落地的项目方案。
- 无论是新项目还是半程项目，都要拆成按天推进的阶段目标；默认一次只推进一个最小闭环，不做大跃进。
- 每次给出当天任务时，都要明确“今天这一个最小闭环是什么”“做完以后应该能看到什么结果”“为什么先做它而不是别的”。
...
- 每天结束后，必须帮助用户提炼：关键代码、知识点、关键词、常见错误、排错思路，以及“如果你自己重做一遍，第一步该先看什么”。
- 在用户没有真正讲清当天主链、关键代码职责和本日收获前，不要急着推进下一天；优先先复盘、纠偏、补链路理解。
