...
def topo_sort_modules(modules: Sequence[object]) -> list[object]:
    ...
    for slot, module in slot_map.items():
        for dependency in _module_requires(module, slot_map):
            in_degree[slot] += 1
            dependents[dependency].append(slot)
...
def inspect_phase(modules: Sequence[object]) -> str:
    ...
    order = "\n".join(
        f"  {index:2d}. {_module_label(module)}"
    ...
    for module in sorted_modules:
        ...
        if requires:
            contracts.append(f"  {module.slot} requires: {', '.join(requires)}")
        if produces:
            contracts.append(f"  {module.slot} produces: {', '.join(produces)}")
    tree = _render_dependency_tree(sorted_modules)
    contract_text = "\n".join(contracts) or "  (none)"
    return f"执行顺序:\n{order}\n\n依赖树:\n{tree}\n\n数据合同:\n{contract_text}"


def _render_dependency_tree(modules: Sequence[SlotModule]) -> str:
    slot_map = {module.slot: module for module in modules}
    children: dict[str, list[str]] = {slot: [] for slot in slot_map}
    in_degree = {slot: 0 for slot in slot_map}
    for slot, module in slot_map.items():
        for dependency in _module_requires(module, slot_map):
            children[dependency].append(slot)
            in_degree[slot] += 1
...
def _render_tree_node(
    ...
    *,
    ...
    lines.append(f"{prefix}{connector}{_module_label(slot_map[slot])}")
...
def _module_label(module: SlotModule) -> str:
    tag = "[B]" if _is_builtin_slot(module.slot) else "[P]"
    return f"{tag} {module.slot}"
...
def _disable_modules_with_missing_module_dependencies(
    ...
    return [
        module
        for module in modules
        if getattr(module, "slot", None) in active
        or not isinstance(getattr(module, "slot", None), str)
    ]
...
def _missing_module_requires(
    ...
    return tuple(
        dependency
        for dependency in (
            str(slot) for slot in getattr(module, "requires", ())
        )
        if _is_module_slot(dependency) and dependency not in active_slots
    )
...
class Phase(Generic[InputT, OutputT, FrameT]):
    ...
    async def run(self, input: InputT) -> OutputT:
        ...
        return cast(OutputT, frame.output)
    ...
    def _validate(self) -> None:
        ...
        for index, module in enumerate(self._modules):
            for raw_slot in getattr(module, "requires", ()):
                slot = str(raw_slot)
            ...
            provided.update(str(slot) for slot in getattr(module, "produces", ()))
