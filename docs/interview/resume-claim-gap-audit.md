...
| Akashic-inspired memory system with retrieval, source references, correction, and forgetting | `amadeus/memory/postgres.py`、`amadeus/memory/retriever.py`、`amadeus/memory/memorizer.py`、`amadeus/tools/recall_memory.py`、`amadeus/tools/forget_memory.py`、`tests/memory/test_memory_retrieval_acceptance.py` | "我把 PostgreSQL + pgvector 长期记忆写入、独立双 lane 检索、source_ref/evidence 回源、更正和遗忘做成了统一闭环；runtime 与工具都通过公开 MemoryEngine 收口。" |
...
| 独立双 lane retrieval、time filters 与 provenance 已可验证 | `amadeus/memory/postgres.py`、`amadeus/memory/retriever.py`、`amadeus/memory/ranking.py`、`migrations/versions/20260711_0004_memory_summary_trgm.py`、`tests/memory/test_memory_retrieval_acceptance.py`、`tests/evaluation/test_evaluators.py` | "vector 与 raw-query lexical 分别查询完整 eligible corpus，再按稳定 memory id 做 RRF。lexical 使用 Akashic ASCII/CJK extractor、参数化 OR-ILIKE 和 pg_trgm GIN；trace 保留每个 lane 的候选数、状态、rank、贡献，以及 hotness/reinforcement/time-decay/emotional-weight 信号。" |
| Akashic-style dual-hypothesis retrieval | `amadeus/memory/retriever.py`、`amadeus/memory/providers.py`、`amadeus/app/bootstrap.py`、`tests/memory/test_memory_retriever.py`、`tests/memory/test_bootstrap_long_term_memory.py` | "显式回答/recall_memory 路径会默认生成 event/general 两条 memory-shaped auxiliary query，和原始 query 一起走向量 lane；关键词 lane 保持 raw-only，失败或超时降级 raw retrieval，结构化 trace 记录生成结果和 fallback。" |
| Runtime/CLI 已公开 memory retrieval trace | `amadeus/runtime/before_turn.py`、`amadeus/runtime/passive.py`、`amadeus/app/cli.py`、`tests/memory/test_runtime_memory.py`、`tests/app/test_cli.py` | "memory recall 的 vector/lexical/union/final candidate counts、lane status、逐记录 RRF contribution、fallback、injected/omitted ids 不藏在 helper 里，而是随 runtime result 与 tool trace 公开。" |
...
- 如果被问哪些是 Phase 2：完整记忆能力，包括 Markdown memory、long-term memory retrieval、embedding、source_ref/evidence 回源、recall/forget、replacement、Akashic-style hotness ranking、Akashic-style dual-hypothesis retrieval、retrieval trace、context-frame injection，以及 focused Phase 2 pytest 证据。产品化 Evaluation runner 和 memory-quality 行为证明是 Phase 3。
