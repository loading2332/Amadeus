...
| Retrieval ranking、time filters、typed memory lanes 已可验证 | `amadeus/memory/ranking.py`、`tests/memory/test_memory_ranking.py`、`tests/memory/test_session_memory_runtime.py` | "当前 retrieval 支持 semantic/lexical 双路、RRF 融合、Akashic-style hotness fusion（0.8 semantic + 0.2 hotness），并把 reinforcement、time decay、emotional_weight 和 final score 写入 trace。" |
