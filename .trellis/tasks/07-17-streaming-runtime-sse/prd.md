...
为 Amadeus 建立可恢复、可观察、与前端框架无关的增量流式链路，使 Web 客户端能按模型实际产生顺序持续获得普通文本与工具活动，并保留最终完整文本作为权威结果。
...
- R1. Provider 支持在仍返回最终 `LLMResponse` 的同时，以异步回调立即发布每个 LLM 步骤产生的普通文本增量；即使同一步随后产生工具调用，已经产生的普通文本也不得缓冲、丢弃或撤回。
...
- R5. SSE 支持首次订阅与断线重连；客户端能够按单调 `seq` 重放累计正文快照与工具事件。统一 reducer 通过相邻累计快照的新增后缀追加文本，并在工具事件处切分 text part，从而恢复工具卡片原本位于文本之间的位置。
- R6. `done` 的最终答案是权威结果，并与已发布增量收口一致；`failed` 保留明确错误和可识别的非完整部分输出。
- R7. 工具调用轮次中通过普通 text/content channel 产生的文本必须照常发布；工具调用分片仍作为工具事件解析，不能误拼进普通文本。
- R8. 产品协议不区分“过程文本”和“最终回答文本”，也不引入 `answer_started` 或事后重分类。供应商通过独立 thinking/reasoning channel 暴露的内容不冒充普通文本；是否增加独立展示类型留给后续 provider capability 任务。
- R9. MVP 通过统一的 typed event envelope 展示工具活动：事件与 `turn_id` 关联，同一次调用的开始和结束共享稳定 `activity_id`，并包含工具名称以及 `started/completed/failed` 生命周期状态。
- R10. 事件顺序必须可验证，使客户端能够按同一条流还原 `text part -> tool activity -> text part` 的实际产生顺序；工具完成后卡片可立即自动折叠，后续文本继续追加。
- R11. 如果 turn 在产生增量后失败，累计部分回答必须保留并以“不完整”语义对外可见，但不得提交为成功 assistant message，也不得触发仅适用于成功回答的记忆提取或 after-turn 副作用。
- R12. MVP 支持持久化的协作式取消：API 接收停止请求，worker 在流式增量或工具步骤边界检测取消并终止后续处理，turn 最终进入独立的 `cancelled` 终态。成功提交前必须经过持久化 `finalizing` 线性化点：在它之前已登记的取消获胜；进入它以后取消返回冲突，不得把已经开始提交的成功消息改写为取消。
- R13. 取消后的部分回答保留并标记为用户取消；系统不得声称已经撤销取消发生前完成的外部工具副作用。
- R14. 同一会话一次只允许一个未终结 turn（`pending/processing/finalizing`）；跨会话的 turn 状态彼此独立，允许所有者在一个会话生成期间切换并使用另一个会话。
...
- R17. 工具生命周期与 text part 事件必须持久化到 PostgreSQL，并使用与 turn 关联的单调序号；首次加载、页面刷新和 SSE 断线重连后均可恢复相同的事件顺序。
- R18. 持久化工具事件至少包含稳定事件类型、`turn_id`、`activity_id`、工具名称、`started/completed/failed` 状态、序号和必要时间戳；事件与所属 turn 采用相同的生命周期策略。
...
- [ ] Provider/runtime 测试证明多个普通文本 delta 按顺序立即发布；即使同一步稍后出现工具调用，先前文本也不会被缓冲或丢弃，最终聚合文本保持一致。
- [ ] Worker/store 测试证明增量与正确 `turn_id` 绑定，序号单调，并在终态后拒绝继续追加。
- [ ] SSE 测试证明处理中可观察累计正文，重连不会要求依赖 API 进程内状态，并在 `done/failed/cancelled` 后关闭。
- [ ] 工具调用测试证明 `text -> tool -> text` 按原顺序发布，工具分片不会混入普通文本，且不需要判断哪一段是“最终回答”。
- [ ] 测试证明 provider 独立 thinking/reasoning channel 不会被错误伪装为普通文本；后续若公开该能力必须新增独立 typed part。
- [ ] 工具活动测试证明开始、完成和失败事件按序可见，且 payload 不包含工具参数或结果。
- [ ] SSE schema 测试证明不同事件种类具有稳定判别字段，React 无需根据可选字段猜测事件类型。
- [ ] 重放测试证明统一 reducer 能以事件 `seq` 把工具前后的累计正文拆成不同 text part；刷新或重连后工具卡片仍位于正确位置，完成态工具默认折叠。
...
- [ ] 取消/完成竞态测试证明 `finalizing` 是唯一线性化点：此前的取消阻止成功消息与 `done`，此后的取消返回 `409` 且成功提交可继续完成。
