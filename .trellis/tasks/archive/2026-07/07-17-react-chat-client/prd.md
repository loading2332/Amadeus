...
- [x] 所有者可以创建、切换和刷新恢复会话，并看到对应历史消息。
- [x] 回答正文随 SSE 增量更新，重连或终态收口不重复内容。
- [x] 工具活动状态与对应 turn 一起显示，且不会泄露参数、结果或原始推理。
- [x] 中途失败的部分回答不会消失，也不会以正常完整回答的视觉状态展示。
- [x] 所有者可以停止正在生成的 turn，UI 最终显示 `cancelled`，并且不会把停止解释为已撤销工具副作用。
- [x] 一个会话生成期间可以切换并使用另一会话；返回后恢复原 turn 的最新流式快照，且不同会话内容不会串流。
- [x] 测试证明普通服务端数据与实时流状态边界清晰：Query 缓存失效不会丢失活动流，SSE 增量也不会制造第二份权威历史记录。
- [x] 失败、空状态、加载状态和窄屏布局具有可验证的用户行为。
- [x] React 公共行为测试、构建检查和 FastAPI 集成测试通过。
- [x] 失败/取消 turn 刷新后仍在时间线中，重试不会覆盖原尝试，也不会给成功回答提供重新生成入口。
- [x] 浏览器断开 SSE 不会发送取消；只有明确点击停止才调用取消端点。
- [x] Markdown 行为测试覆盖 GFM、代码复制、流式未闭合语法、原始 HTML 禁用与危险链接处理。
- [x] 响应式测试覆盖桌面固定侧栏、窄屏 Drawer、底部输入可达性，以及表格/代码块局部滚动而非页面横向溢出。
- [x] 主题测试覆盖系统偏好、手动覆盖、刷新恢复以及浅色/深色下主要状态的可读性。
- [x] 组件与构建检查证明 MUI 是唯一主要组件/主题系统，未引入 Tailwind、shadcn/ui 或 MUI X。
- [x] 首条消息后会话标题由服务器确定性生成并在刷新后保持一致，过程不调用 LLM。
- [x] 输入测试覆盖 Enter、Shift+Enter、中文 IME composition、空白阻止、重复提交阻止、生成中停止按钮和移动端显式发送。
- [x] 生产构建产物可由 FastAPI 同源返回，本地 Vite 代理可连接 HTTP 与 SSE，且无生产 CORS 依赖。
- [x] 零残留检查证明旧原生 HTML/JS/CSS、专属选择器和兼容入口均已删除；访问 `/` 时只能得到 React 构建入口，缺少构建产物时明确失败而不是回退旧页面。
- [x] Vitest 覆盖 API guard、Query/live overlay 合并、SSE seq 去重、Markdown、Composer/IME 和主要组件状态；运行快速且不依赖真实浏览器或网络。
- [x] Chromium Playwright 使用确定性 FastAPI/PostgreSQL/流式 worker fixture 覆盖：创建并流式完成、跨会话恢复、停止保留部分回答、失败刷新重试、窄屏 Drawer/主题/Markdown 溢出；失败时保留 trace，测试不调用真实付费 LLM。
- [x] API client 测试证明普通 HTTP 统一经过 Axios instance 并正确转换非 2xx、网络错误、取消和安全错误 payload；组件不直接调用 Axios/fetch，SSE 仍由独立 EventSource 管理。
- [x] `kill-ai-slop` 审计提供扫描与人工 triage 报告；经批准的修复完成后重新扫描，并通过桌面/移动端视觉 QA。保留的命中必须说明其明确设计意图，不得仅为通过扫描而隐藏。
